(function(){
  ...
  // Expose setupButtons so main script can call it. This copies the original setupButtons logic
  // but reads DOM elements at runtime (so it is safe to call after DOMContentLoaded).
  function setupButtons() {
    ...
    if (btn9) {
      btn9.addEventListener('click', (e) => {
        const btn = e.target; btn.classList.toggle('loading');
      });
    }

    // Setup per-button hover handlers (uses animToggle from DOM at runtime)
  ...
  window.App.setupButtons = setupButtons;
})();
