(function(){
  // Lightweight helpers namespace
  window.App = window.App || {};
  const helpers = {};

  helpers.normalizeHex = function(val) {
    if (!val) return '#000000';
    val = val.trim();
    if (val.startsWith('#')) return val;
    if (val.startsWith('rgb')) return helpers.rgbToHex(val);
    return val;
  };

  helpers.rgbToHex = function(rgb) {
  ...
  helpers.getContrastColor = function(r, g, b) {
    const brightness = (r * 299 + g * 587 + b * 114) / 1000;
    return brightness > 186 ? '#000000' : '#ffffff';
  };

  helpers.applyMainPanelBounds = function(mainPanel) {
    // computes and applies inline bounds so center animated content doesn't underlap panels
