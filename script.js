document.addEventListener('DOMContentLoaded', () => {
	...
	// Initial setup (default view is set below once DOM refs are ready)
...
`;
	...
	// --- Button setup and per-button effects ---
...
// collect unique variable names and store defaults
	...
	// --- Animated view extractor: pulls the large inline block into a named function ---
	function setupAnimatedViewControls() {
		// Animated view setup
		// --- Animated view: line creation and controls ---
		...
		function updateLinePositions() {
			...
			// prefer the animated color picker if present, fall back to the global colorPicker or CSS var
			const animPicker = document.getElementById('animated-color-picker');
			const color = (animPicker && animPicker.value)
				? animPicker.value
				: ((colorPicker && colorPicker.value) ? colorPicker.value : getComputedStyle(root).getPropertyValue('--animated-bg').trim() || '#ffffff');
		...
		// insert animated line color picker into the right-top panel dynamically
		if (rightTopEl) {
			const animColorRow = document.createElement('div');
			animColorRow.className = 'color-picker-row';
			animColorRow.innerHTML = `<label>Line Color <input id="animated-color-picker" type="color" value="#ffffff"/></label>`;
			const btnArea = rightTopEl.querySelector('.panel-buttons');
			if (btnArea) rightTopEl.insertBefore(animColorRow, btnArea);
			else rightTopEl.appendChild(animColorRow);

			// initialize picker from CSS var and wire updates
			...
			if (acp) {
				...
				acp.addEventListener('input', (e) => {
					root.style.setProperty('--animated-bg', e.target.value);
					updateLinePositions();
				});
			}
		}
			...
			h1.textContent = 'Button Showcase';				// Dynamically update right panel content
				...
				setTimeout(() => {
					...
					if (targetSelect && colorPicker && applyBtn && resetBtn) {
						...
						// Animated view: delegate to extracted setup function
